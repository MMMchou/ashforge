...
func ClearProfileCache(modelID string, sys *hw.System) error {
...
// └─────────────────────────────────────────────────────────────────┘
func Warmup(profile *catalog.DeployProfile, binaryPath, modelPath string, sys *hw.System, fast bool, modeOverride string) (*OptimizedProfile, error) {
	fingerprint := sys.Fingerprint()
	...
	if sys.ClusterCaps().HasBlackwell {
	...
	// RAM safety check
	if warning := checkRAMSafety(sys, profile); warning != "" {
		fmt.Printf("      %s\n", warning)
	}
	...
	if profile.CtxOverride > 0 {
		...
		args := BuildArgs(profile, binaryPath, modelPath, port, sys, ctxFixed, batchSize, ubatchSize)
		...
		if profile.Mode == "moe_offload" || profile.Mode == "moe_partial" {
			...
			ideal := IdealStartCtx(profile, sys)
		...
		for attempt := 1; attempt <= 8; attempt++ {
			...
			args := BuildArgs(profile, binaryPath, modelPath, port, sys, ctxTry, batchSize, ubatchSize)
			tps, vram, err := runBenchmarkRound(binaryPath, args, port)
		...
		if bestCtx > 0 && failedCtx > bestCtx {
			...
			for hi-lo > 4096 {
				...
				args := BuildArgs(profile, binaryPath, modelPath, port, sys, mid, batchSize, ubatchSize)
				tps, vram, err := runBenchmarkRound(binaryPath, args, port)
				if err != nil {
					fmt.Printf("OOM\n")
	...
	if minBW := sys.ClusterCaps().MinBandwidth; minBW > 0 && minBW < 200 {
	...
	for _, ub := range ubatchCandidates {
		args2 := BuildArgs(profile, binaryPath, modelPath, port, sys, bestCtx, 512, ub)
	...
	fmt.Println()
	// Phase 2 结果覆盖 Phase 1（仅在 ubatch 调优确实更快时）
	if ubBestTPS > bestTPS {
		bestTPS = ubBestTPS
		bestArgs = ubBestArgs
		bestVRAM = ubBestVRAM
	}
	...
	modes := deriveModeProfiles(probeResults, profile, modelPath, port, sys, batchSize, ubatchSize)
...
func BuildArgs(profile *catalog.DeployProfile, binaryPath, modelPath string, port int, sys *hw.System, ctxSize, batchSize, ubatchSize int) []string {
	vramMB := sys.TotalVRAM_MB() // 多卡总VRAM
	...
	useMlock := ShouldMlock(sys, profile)
	...
	args := []string{
		...
		"--threads", strconv.Itoa(ThreadsForMode(profile.Mode, sys)),
	...
	if !sys.ClusterCaps().HasBlackwell && sys.GPUCount() <= 1 {
	...
	if sys.GPUCount() > 1 {
		if sys.HasNVLink() && SupportsGraphSplit(binaryPath) && profile.Arch != "moe" {
			args = append(args, "-sm", "graph")
		} else if ts := sys.TensorSplitArg(); ts != "" {
	...
	if !useMlock && ShouldMmap(sys, profile) {
...
func checkRAMSafety(sys *hw.System, profile *catalog.DeployProfile) string {
	totalMB := sys.RAM.Total_MB
	freeMB := sys.RAM.Free_MB
...
func deriveModeProfiles(results []ProbeResult, profile *catalog.DeployProfile, modelPath string, port int, sys *hw.System, batchSize, ubatchSize int) []ModeProfile {
